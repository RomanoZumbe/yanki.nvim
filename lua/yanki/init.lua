...
---@param parm table the settings for yanki
function M.setup(parm)
    M.settings = (parm or {})

    -- Either start monitoring the system clipboard or add an autocommand
    -- for yanking in neovim
    if M.settings.system_clipboard and M.settings.system_clipboard.observe then
        sysclip.ObserveSystemClipboard(M.settings.system_clipboard.command,
                                       M.settings.system_clipboard.args,
                                       M.settings.system_clipboard.wait,
                                       M.AddToHistory)
    end

    if M.settings.system_clipboard.yank_observe then
...
--- Applies all active transformers to the given string and adds it to the history
---@param text string the string to add to the history
function M.AddToHistory(text)
    ...
    for _, transformer in ipairs(M.settings.transformer or {}) do
        if transformer.active then
            local newLines = {}
            for i = 1, #lines do
                local transformedText = transformer.action(lines[i])

                -- NOTE:The transformer can return a string or a table of strings
...
-- Put the complete yank history in the current buffer
function M.PutAll()
    if #M.yanks == 0 then return end
    for _, yank in ipairs(M.yanks) do M.Put(yank) end
end

--- Put text into the active buffer at the current cursor position
---@param content string text to put into the buffer
function M.Put(content)
    ...
    if M.settings.PutInNewLine or string.sub(content, -1) == "\n" then
